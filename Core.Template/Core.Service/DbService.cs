...
namespace Core.Service
{
    public class DbService : IDbService
    {
        ...
        public PageResult<T> QueryPage<T, S>(Expression<Func<T, bool>> funcWhere, int offset, int limit, Expression<Func<T, S>> funcOrderby, bool isAsc = true) where T : class
        {
            var list = this.Set<T>();
            if (funcWhere != null)
            {
            ...
            {
                rows = list.ToList().Skip((offset - 1) * limit).Take(limit).ToList(),
                total = list.Count()
            };
            return result;
        }
